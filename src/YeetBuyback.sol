...
/// @title YeetBuyback
/// @notice Receives 100% of protocol fees (native USDC) and can do exactly one thing with them: buy the protocol
///         token and burn it. There is no withdraw. The token is set once and is immutable afterwards.
///         Buys from the bonding curve while the token is on it, from its Uniswap v4 pool after graduation.
contract YeetBuyback is IYeetBuyback, IUnlockCallback, Ownable2Step, ReentrancyGuard {
    ...
    /// @notice Permissionless: spend up to `maxUsdc` of the balance buying the protocol token and burning it.
